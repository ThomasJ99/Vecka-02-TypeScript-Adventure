...
const startingInventory: string[] = ["Svärd"];

console.log(
  `Välkommen ${heroName}! Du börjar med ${hp} HP och ett ${startingInventory}.`
);
...
const enemies: string[] = ["Slemmpropp", "Varg", "Drake"];
...
for (const enemy of enemies) {
  ...
  if (enemy === "Drake") {
    if (hp >= 50 && isAlive && inventory.includes("Svärd")) {
      console.log(
        `Du besegrade draken! och vann spelet! Du fick en hälsodryck som sätter ditt hp till ${(hp = 100)}`
      );
    } else {
      isAlive = false;
