...
console.log(`Du går ner i en grotta och stötter på en gammal man, 'Det är farligt att gå ensam, ta det här' `);
...
const enemyDrops = ["Trä sköld", "Boomerang", "Pilbåge", "Bomber"];
// Enemy skada
...
for (const enemy of enemies) {
    if (isAlive) {
        ...
        if (enemy === "Octorok") {
            ...
            hp += hjärta;
            console.log(`Du fick en ${enemyDrops[2]} och ett ${healingDrops[0]} som ger dig mer hp. Din hp är nu ${hp}`);
        ...
        if (enemy === "Tektite") {
            console.log(`Tektite önskar att den hade stött på vad som helst förutom dig.`);
        ...
        if (enemy === "Peahat") {
            if (inventory.includes("Pilbåge")) {
                ...
                hp -= enemyDmgStr;
                console.log(`Du kunde inte skjuta ner den flygande fienden och behövde springa därifrån efter du tagit ${enemyDmgStr} skada, din hp är nu: ${hp}`);
            }
        }
        if (enemy === "Dodongo") {
            if (inventory.includes("Bomber")) {
                console.log(`Tur nog hittade du bomber tidigare, du eliminerade Dodongo utan problem med dem`);
                inventory.splice(3); // Tar bort bomberna
            }
            else {
                hp = 0;
                isAlive = false;
                console.log(`Du hade ingen metod för att döda bossen, din hp är ${hp}. Game Over`);
            }
        }
        if (enemy === "Ganon") {
            if (hp >= 4 ||
